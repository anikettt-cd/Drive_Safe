...
class _DrowsinessDetectionPageState extends State<DrowsinessDetectionPage> {
  ...
  // ---> NEW HARDWARE WI-FI CONFIGURATION <---
  // UPDATE THIS IP ADDRESS AFTER UPLOADING CODE TO ESP8266
  ...
  Future<void> _initializeSystem() async {
    await [Permission.camera, Permission.location].request(); // Removed BT permissions
  ...
  // ---> NEW HARDWARE SIGNAL FUNCTION (WI-FI) <---
  Future<void> _sendWiFiCommand(String endpoint) async {
    final url = Uri.parse('http://$esp8266IP/$endpoint');
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 2));
      if (response.statusCode == 200) {
        debugPrint("Wi-Fi Signal '$endpoint' sent successfully to ESP8266!");
      }
    } catch (e) {
      debugPrint("Wi-Fi Error: Could not reach ESP8266. Check hotspot connection. -> $e");
    }
  }
